...
2. **Extraction Phase:** The script automatically runs both the baseline RLM extractor and the Propositional KG extractor using the scrubbed input. It writes their outputs into the `working/evaluations/[name]/` directory.
3. **Evaluation Phase:** 
   - Checks if `answer_key.json` exists in the evaluation directory. If not, generates it from the scrubbed input and caches it.
   - Tests both extracted KGs against the answer key.
